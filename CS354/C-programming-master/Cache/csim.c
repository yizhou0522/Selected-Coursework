 ...
 */
void init_cache() {
    // get the size of S and B
    S = pow(2, s);
    B = pow(2, b);

    cache_line_t line;

    //malloc size for simulated cache
    cache = (cache_set_t*) malloc(sizeof(cache_set_t) * S);
    if(cache==NULL)
    {
        fprintf(stderr,"Error: malloc returns null");
        exit(1);
    }
    for (int i = 0; i < S; i++) {
        //malloc for lines
...
void access_data(mem_addr_t addr) {
    ...
    for (int line = 0; line < E; line++) {
        ...
        if (cache[set][line].valid && cache[set][line].tag == tag) {
            hit_cnt++;
            cache[set][line].counter = counter++;
            return;
        }
    }
    ...
    int lru_index = 0;
    for (int line = 0; line < E; line++) {
        if (cache[set][line].counter < cache[set][lru_index].counter) {
            lru_index = line;
        }
    }
    cache[set][lru_index].tag = tag;
    cache[set][lru_index].counter = counter;
}
